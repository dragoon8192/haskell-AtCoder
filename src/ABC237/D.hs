...
main = runSolver do
  n :: Int <- parseLine
  s :: String <- BS.unpack <$> parseLine
  let
    ans = L.foldr func (Seq.singleton n) $ zip [0..] s
    func (i, 'L') seq = seq Seq.|> i
    func (i, _  ) seq = i Seq.<| seq
  liftIO $ putStrLn . unwords . map show . toList $ ans
