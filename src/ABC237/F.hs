...
main :: IO ()
main = runSolver do
  (n, q, x) :: (Int, Int, Int) <- parseLine
  ps :: UV.Vector Int <- parseLineWithN n
  clrs :: [(Int, Int, Int)] <- parseLinesN q
  let
    qUpdate v (1, l, r) = v UV.// diffA
      where diffA = zip [l - 1 .. r - 1] $ IS.toAscList . IS.fromList . UV.toList . UV.slice (l - 1) (r - l + 1) $ v
    qUpdate v (_, l, r) = v UV.// diffA
      where diffA = zip [l - 1 .. r - 1] $ IS.toDescList . IS.fromList . UV.toList . UV.slice (l - 1) (r - l + 1) $ v
    ans = L.foldl qUpdate ps clrs
  liftIO $ print $ maybe 0 (+1) $UV.elemIndex x ans
